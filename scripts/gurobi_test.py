...
def fixed_N_qcqp(N, dose_deposition_dict, constraint_dict, radbio_dict, S, alpha_smoothing):
	...
	obj = -N*(alpha*gb.quicksum(target_dose) + beta*target_dose@target_dose) + alpha_smoothing*smoothing_term@smoothing_term
	m.setObjective(obj)
	...
	for oar in OAR_names:
		...
		if constraint_type_dict[oar] == 'mean_dose':
			...
			mean_constr = N*(gamma*gb.quicksum(oar_dose) + delta*oar_dose@oar_dose)
